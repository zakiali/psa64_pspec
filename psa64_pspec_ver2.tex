...
secondary temperature fluctuations generated by the kinetic Sunyaev-Zel'dovich effect \citep{mesinger_et_al2012,zahn_et_al2012,george_et_al2014}.
...
$0.15<k<0.5\hMpci$ at $z=8.4$.  This result places significant constraints on the 
...
I.  We take the conservative approach and do not correct for this effect, noting that the leakage of Q in to I will result in positive power, increasing our limits.
...
bins by the r.m.s. of the beam response, we can get a net increase in sensitivity.  
...
declination of the array. For PAPER, this filter roughly douples the
sensitivity of our measurements.  It is also worth noting that, like delay
...
We implement the optimal filter by weighting the fringe-rate bins on the sky by
the r.m.s. of the beam at that same location.  In order to obtain a smooth
filter in the fringe-rate domain, we fit a Gaussian with a hyperbolic tangent
tail to this filter. We convolve the time-domain visibilities with the Fourier
transform of this fringe-rate filter to produce an averaged visibility.  The
effect on the data can be seen in Figure \ref{fig:waterfalls}. There is an
associated signal loss with the this optimal-fringe filter because we are
infact removing signal. In conjucnction with the cross-talk filter described
above, we find that there is a 39\% signal loss.
...
fringe-rate filter that is a flat  weighting, $f_{\text{flat}}$, versus that
with an optimal filter weighting, $f_{optimal}$, as described above. We apply
these filters to random noise with variance 1, without loss of generality. We
can conclude that 
[XXXcheck math] 
...
Equation \eqref{eqn:frf_inttime} evaluates to 1886 s for an optimal fringe-rate
filter on the fiducial baseline at 150 MHz.  Over the range of frequencies used
in our power spectrum estimate, the integration times range from 1936 s to 1790
s, corresponding to an average of 31 minutes. As a rough check, we note that
this is roughly the time it takes a source to transit through one fringe
period.  Because the fringe-rate filter integrates in time, the number of
statistically independent samples of the sky drastically decreases from 83 to
$\sim2$ independent samples per hour.
...
calcutation. We note that at 7:00 LST, there is an increase in the system temperature due to the rising of the galactic plane as seen in Figure \ref{fig:coverage}.
...
heating beyond what some fiducial models predict.  We refer the reader
...
As sensitivity improves, polarization leakage will become a foreground of increasing concern.
Independent fringe-rate filtering of the XX and YY polarizations
...
to show that limits of $T_s>4$ K are fairly generic for models with
...
presented in a forthcoming paper \citep{pober_et_al2015}.
%In that paper,
