...
a 4.5-month observing campaign with a 64-element deployment of the Donald C. Backer
...
$2\sigma$ upper limit on $\Delta^2(k)$ of ($20.6~{\rm mK})^2$ in the range
$0.1<k<0.5\hMpci$ at $z=8.4$.  This represents a four-fold improvement
...
emission from these foregrounds is brighter than the predicted
reionization signal by more than 5 orders of magnitude
...
be solved for on the basis of redundancy (P14;P12a;\citealt{zheng_et_al2014}).
...
antennas, cables, and receivers. 
Note that redundant calibration only constrains the relative complex gain solutions
between antennas and is independent of the sky. Since redundant
calibration preserves signals common to all redundant baselines,
this type of calibration is lossless. 
...
from the true value, LOGCAL has the shortcoming of being a biased by the assymetric behavior
of additive noise in the logrithm \citep{liu_et_al2010}.
...
Implementaion of redundant calibration was done with the 
...
\caption{
%[XXX what are units of color scale?] There are none. It represents different
%redundant baselines.
...
%[XXX is this before or after compression?  df and dt are one of each.] After. I
%dont understand this.
} \label{fig:chi2}
\end{figure}

[XXX from here to the end of section is too hard to follow; don't understand].
...
as illustrate in 
Figure \ref{fig:chi2} for one day of observation.
...
$\frac{2}{\text{DoF}}$. The distribtuion of the $\chi^{2}$ in figure
...
After solving for the relative complex gains of the antennas using redundant calibration, an
overall phase and gain calibration remains unknown. We use the standard self
calibration method for radio interferometers to solve for the absolute
phase calibration. We used Pictor A, Fornax A, and the Crab Nebula to
...
array is essentially instantaneously coplanar.  A source spectrum is derived for
...
indicating catalog positions \citep{XXX}. Image was synthesized with two hours
...
scale of the PAPER-64 observations. Error bars indicate the 68\% confidence
...
white noise with a uniformly varying phase, for each baseline and independent
time mode. The total number of independent
modes we average over before fitting a 9th degree polynomial to the simulated
signal is 1017, which includes the number of unique baselines and the number of
...
signal loss decreases for higher $k$'s.
...
P12b and \citet{petrovic_oh2011}. The delay spectrum filtering technique is well suited to the maximum redundancy PAPER configuration which is not optimized for the other approaches where high fidelity imaging is a pre-requisite.   The delay spectrum foreground clean method is described in detail by P14; its application is unchanged here.  In summary; we fourier transform each baseline spectrum into the delay domain  
...
\tilde{V}(\tau) &= \int{W(\nu)A(\nu)B(\nu)I(\nu)
                ...
                &= \int{W(\nu)A(\nu)
                   e^{-2\pi{i}\nu(\tau_{g}-\tau)}d\nu} \\\notag
                &= \tilde{W}(\tau) \ast \tilde{A}(\tau) \ast
                   \tilde{I}(\tau) \ast
                   \delta(\tau_{g} - \tau),
\end{align}
last one] where $A(\nu)$ is the frequency dependent beam, $W(\nu)$ is a
Blackman-Harris taper function to minimize band edge effects along with the
frequency sampling function due to RFI flagging, and $I(\nu)$ is the source
spectrum.  In delay domain, a smooth spectrum source is a delta function at
delay $\tau_{g}$ (a flat spectrum source's delay spectrum) convolved by the
spectrum shape, the beam, the bandpass shape, the taper function and the
sampling function. The source shape, $A(\nu)$, which includes the bandpass,
imprints a width onto the delta function and leads to leakage of power outside
...
Applying the delay filter to the 30 m East-West baselines used in the power spectrum analysis,
foregrounds are suppressed by $\sim$5 orders of magnitude in power, or
 -50 dB of foreground suppression, as seen in Figure
...
.0015\% for the higher modes.  [XXX are we applying these numbers to the pspec or just calling it negligible?]  

After the wideband delay filter, we conduct another remove a second layer of RFI and crosstalk
...
coherent $k$-modes on the sky before squaring to get the maximum sensitivity and is mathematically similar to the more traditional process of gridding in the $uv$ plane, but just for a single baseline.  However, rather appealing a box-car average, we can apply a kernel --a so-called 'fringe-rate' filter-- that
weights different temporal rates by the antenna beam corresponding to the parts of the sky moving at the same rate.

Broadly, for a given baseline and frequency, different parts of the sky
correspond to different fringe-rates.
...
and have static fringe patterns \cite{parsons_backer2009}. We then compute the average beam response for each contour of constant fringe rate to construct our time average kernel or "fringe rate filter".
...
Applying this filter effectively weights the data by another factor of the beam area. By utilizing prior knowledge about the beam area we are selectively down weighting areas on the sky contributing little signal. This results in an SNR boost of about 2.  The net boost
in SNR depends on the shape of the beam and array declination and is functionally equivalent to the beam$^2$ weighting used in wide-field mosaicing. It is also worth noting that, like the delay spectrum filtering, fringe-rate filtering is
 implemented on a per-baseline basis.
...
gaussian with a hyperbolic tangent tail to this filter. %as shown in figure \ref{fig:fringe_rate_cut}. 
% XXX too much detail
...
integration.  The before/after effect on the data can be seen in Figure \ref{fig:waterfalls}.

The PAPER beam is ~45 degrees FWHM, and the array is located at a declination
...
%Figure \ref{fig:fringe_rate_cut} shows a cut of the optimal fringe-rate filter at 159 MHz for
...
\end{figure}
The action of the fringe rate filter is to get a nearly optimally weighted sample, averaged at the longest 
physically possible integration time.
...
integration times range from 1936 s to 1790 s, corresponding to an average of 31
minutes. As a rough check, note that this is roughly how long a single 30m fringe at 150MHz takes to make one oscillation on the sky.  Because the fringe-rate filter integrates in time, the number
of statistically independent samples of the sky drastically decreases from 83 to $\sim2$ independent samples per hour.
...
are no systematics in time. 
[XXX fix this known issue.]
However, we do note that on the 2 day time scale,
there is a slight increase power owning the fact that neighboring days are
slightly correlated with each other.  

Figure \ref{fig:stability} shows the
...
$T_{\rm rms}$ is the RMS temperature.
...
of the system temperature of the PAPER instrument. Agreement between the instantaneous measurements of the system temperature, the LST repetition variance, and the predicted power spectrum noise level (see below) indicates a robustly stable system with no significant long term instability contributing appreciable noise.
...
estimatore formalism to measure the power spectrum. The form of our estimator is
...
$Y$ are the constants that convert from angles and frequency to the comoving
...
spectrum in this was is that delay modes are a very good approximation to line
of sight fourier modes, which holds for the short 30 m baselines used in this
analysis (P12b). Due to the chromaticity of an interferometer, this is in
general not valid assumption and depends on the lenght of the baseline.
...
array. In estimating the power spectrum we use specific redundant baseline,
applying the analysis to different redundant baselines separtately. In the
description what follows we use the the east-west 30 meter redundant baselines (
baselines $49\_41$, $41\_47$, $10\_3$, etc. in figure
\ref{fig:antenna_positions}), but we apply the same anaysis to the 30 m
baselines with one row spaceings (10-41,9-3,3-47,...  and 49-3,10-58,41-25,...
antennas in figure \ref{fig:antenna_positions}), combining the final power
spectra in the bootstrapping after estimating power spectra.  Focusing on the 30
m east-west redundant baselines, our data vector becomes a list of visibilities
such that, 
...
in Section \ref{sec:bootstrap}, we use bootstrap over all of these independent measurements,
...
\ref{eqn:d2pspec_model} and \ref{eqn:patchy_bound} in the limit
that $T_{\rm s}<T_{\rm CMB}$.
Color indicates $T_{s}$ (in K).
[XXX put K units in plot next to colorbar]
[XXX remove kmin > 0.5 where we have no constraints].
...
Our results become more interesting in the range of $\kmin\sim0.1$ and $\kmax\sim30$
representative of fiducial simulations \citep{lidz_et_al2008,zahn_et_al2007}.
For neutral fractions of 0.3, 0.5, and 0.7, we find
we find that $\Tspin\gtrsim 5\,\text{K}$.  As discussed in \citet{pober_et_al2015}
in the context of simulations that do not rely on the approximations
we have made, these limits push the bounds of the amount of heating possible
given the currently observed star formation rates in high-redshift galaxy populations 
\citep{bouwens_et_al2014,mcleod_et_al2014}
and assumptions about the relationship between star formation rates and
X-ray luminosities \citep{furlanetto_et_al2006,pritchard_loeb2008,fialkov_et_al2014}.
Assuming the midpoint of reionization lies close to $z=8.4$ (a
reasonable assumption given that \citealt{planck_2015} suggests
a midpoint of $z=8.8$), these bounds provide evidence
for additional sources of heating beyond the observed
galaxy population.  We refer the reader to \citet{pober_et_al2015} for a
