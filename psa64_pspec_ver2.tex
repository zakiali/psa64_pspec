...
\tilde{V}(\tau) &= \int{W(\nu)S(\nu)A(\nu)B(\nu)I(\nu)
                ...
                &= \tilde{W}(\tau) \ast\tilde{S}(\tau) \ast \tilde{A}(\tau) \ast
                   ...
                   \delta(\tau_{g} - \tau),
\end{align}
[XXX too many variables here.  S goes in W, B goes in A, lose numbers except last one]
...
the source spectrum. 
In delay domain, a smooth spectrum source is a delta
...
The dashed
vertical lines at $0.6\hMpci$ show the bounds of the
...
$\Tsys=500K$.
XXX describe triangles and cite
XXX Using 100 bootstraps. Correction factors are fringe-rate
filter (1.9), signal loss (1.02), and median statistic (1.4), delay filter signal loss, bandpass signal loss.}
...
The optical depth is given by \citep{zaldariagga_et_al2004}
...
bubble sizes, $k_{\text{min}}$ and $k_{\text{max}}$, respectively:
\begin{equation}\label{eqn:patchy_bound}
    \Delta^{2}_{i}(k) = (x_{\text{HI}} -
x_{\text{HI}}^{2})/\ln{(k_{\text{max}}/k_{\text{min}})}.
\end{equation}
For combinations of $k_{\text{min}}$ and $k_{\text{max}}$,
we 
