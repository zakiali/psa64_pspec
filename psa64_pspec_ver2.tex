...
\altaffiltext{5}{School of Earth and Space Exploration, Arizona State U., Tempe,
AZ}
...
hydrogen during the EoR and the dark ages. GMRT (\cite{paciga_et_al2011}),
...
(\cite{burns_et_al2012}) , and SciHi (\cite{tabitha_et_al2014}) seek to measure the
...
foregrounds are spectrally smooth, many techniques have been developed to use
...
%XXX: Last sentence is weird. You don't develop techniques because the foregrounds
%are spectrally smooth. You develop techniques because the foregrounds are bright
%and contaminating. The smoothness is just what enables you to do it.

Currently, the best upper limit on the 21 cm power spectrum is $\Delta^2 (k) \leq (41\,\textrm{mK})^{2}$ at $k=0.27 h \text{Mpc}^{-1}$, measured by a 32-antenna element version of PAPER (XXX: cite PAPER paper). However,
...
the Epoch of Reionization (PAPER) -  64 antenna dual polarization maximum
redundancy configuration. XXX: Sell it more, and advertise Jonnie's paper.
...
the center of the galaxy). Due to LST drift of 3:56 minutes every solar day, we
...
\cite{parsons_et_al2009} and \cite{parsons_et_al2014}, we deconvolve the
...
with this type of filter is the same as in \cite{parsons_et_al2014} because we
...
optimal quadratic estimator method to measure the power spectrum. (XXX: Rephrase
the previous sentence. Divided into what?)
We first
...
\end{equation}
For a data vector $\x$, the optimal quadratic estimator is given by [XXX: make clear
that this is not the power spectrum yet. I would refrain from calling this the optimal
quadratic estimator (optimal quadratic estimator of what?). Instead, state that this is
an intermediate quantity that will be needed to form our optimal quadratic estimator
of our power spectrum]
\begin{equation}
\label{eqn:quad_est}
...
estimate of the power spectrum in bin $\alpha$ [XXX: make clear bins in what "space", i.e.
these are bins in k] is given by
\begin{equation}
    \hat{p}_{\alpha} = \mathbf{M}\hat{q}_{\alpha}, 
\end{equation}
[XXX: This equation has mixed notation, because you're doing half of it in bold vector/matrix
notation and half writing out components. Pick one. If the latter, it ought to be beta on the right
hand side, not alpha]
...
product of our data vector with itself, $\mathbf{C} = \expval{\x^{t}\x}$, [XXX: the transpose
should be on the other copy of x. Also, I think we should define the angular brackets as
the ensemble average, and then say that because we can't do an ensemble average,
we approximate it using a time average]
...
$\mathbf{C}$ with respect to $\alpha$ [XXX: you can't differentiate with respect to an index!
I think you mean differentiating w.r.t. $p_\alpha$] ($\frac{\partial C}{\partial \alpha}$). The
...
constructing an unbiased estimator and therefore ignore this term. [XXX: Careful. The cross-correlation
gets rid of the noise bias, but there's a foreground bias. Let's be careful and define the
bias here as the instrumental noise bias. Also, since we're talking about cross-correlations,
I think we should at some point say explicitly that the copies of $\mathbf{x}$ in the estimator
are to be replaced with $\mathbf{x}_1$ and $\mathbf{x}_2$.]

The normalization matrix is defined such that
\begin{equation}
    \mathbf{W} = \mathbf{M}\mathbf{F}, 
\end{equation}
where $\mathbf{F}$ is the Fisher information matrix, given by $F = cov(\hat{q})
= 2tr(C^{-1}Q_{\alpha}C^{-1}Q_{\beta})$ [XXX: several things. First, use the proper bold fonts even
if it's in the text. Everything in the trace should be bolded. Also, the $F$ needs to have indices
$\alpha$ and $\beta$. The 2 should be a $1/2$, and the $\textrm{Cov}(\hat{q})$ is something I'd omit here. It's true, but it's less fundamental than the rest of the expression] and $\mathbf{W}$ is 
...
$\alpha$'th bin. [XXX: I suggest being more precise here, and actually writing out the
relation between phat and p]
...
power spectrum in the $\alpha$'th bin is coupled with every other band power. [XXX: Umm...no? I don't think it does]
...
functions. [XXX: we need to be much more explicit here. For $\mathbf{L}$ to have the
behavior we want, we need to order the data in $\mathbf{x}$ in a very particular order. We
should also be talking about the error correlations in addition to the window functions]
...
%move this to discussion section?
