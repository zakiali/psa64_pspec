...
Here we briefly review the optimal quadratic esitimator (OQE) formalism with on
emphasis on an application to our data. The goal here is to make an estimate of
...
\end{equation}
Then for a data vector $\x$, the optimal quadratic estimator is given by 
\begin{equation}
\label{eqn:quad_est}
    \hat{q}_{\alpha} = \frac{1}{2}\x^{t}\mathbf{C}^{-1}\mathbf{Q}_{\alpha}\mathbf{C}^{-1}\x - b_{\alpha},
\end{equation}
up to some normalization matrix $\mathbf{M}$, such that the estimate of the
power spectrum is given by 
\begin{equation}
    \hat{p}_{\alpha} = \mathbf{M}\hat{q}_{\alpha}, 
\end{equation}
where , in our analysis, $\x$ is the set of visibilities for all redundant
baselines of a certain type, $\mathbf{C} = \expval{\x^{t}\x}$, $\mathbf{Q}$ is
the derivative of $\mathbf{C}$ with respect to $\alpha$, and $b_{\alpha}$ is a
bias to the estimate. The normaliztion matrix is defined such that
\begin{equation}
    \mathbf{W} = \mathbf{M}\mathbf{F}, 
\end{equation}
where $\mathbf{F}$ is the Fisher information matrix and $\mathbf{W}$ is defined
as the window function matrix.
...
In order to show the degree of signal loss due to our empirical estimates of the
covariances, we run monte carlo simulations. These simulations consists of
injecting a model eor signal on top of the the data and run through our power
spectrum estimator. We then compare the output power spectrum to the true power
spectrum of the input model to determine the signal loss. The eor model used is
a simulation of a random eor signal in fi        
