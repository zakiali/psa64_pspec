...
Gianni Bernardi\altaffilmark{3,13,14}, 
...
Cambridge, MA}
\altaffiltext{13}{Dept. of Physics and Electronics, Rhodes University}
\altaffiltext{14}{Harvard-Smithsonian Center for Astrophysics}
...
upper limit on $\Delta^2(k)$ of ($19.7~{\rm mK})^2$ in the range
...
the first galaxies \citep{lidz_et_al2008}, their ultraviolet luminosities [XXX need a cite],
the abundance of X-ray sources and other sources of heating \citep{pritchard_loeb2008,mesinger_et_al2013},
and higher-order cosmological effects like the relative velocities of baryons and dark matter \citep{visbal_et_al2012,mcquinn_oleary2012}.
...
spin temperature of the IGM, and as is shown in a companion to this paper,
...
The first stage uses instantaneous redundancy to solve for the majority of the 
per-antenna internal degrees of freedom in the array.  In the second stage, standard self-calibration is used 
...
OMNICAL\footnote{https://github.com/jeffzhen/omnical} package, an open-source
redundant calibration package, which can be easily adapted to other instruments
other than PAPER.  \citep{zheng_et_al2014}. This package implements both LOGCAL
...
\caption{
Log of $\chi^{2}/\text{DoF}$ of all baseline residuals after the application of OMNICAL.
...
\end{align} 
and effectively comes out to the number of visibilities the
$\chi^{2}$ is calculated over. If the data are completely noise dominated, the
...
demonstrating the stability of the PAPER instrument. The mean of the $\chi^{2} = 1.9$, leading us to believe we are not thermal noise dominated at this point in the analysis. However, further removal of systematics will occur along the analaysis.
...
redundant baselines are not redundant with one another, owning to some
...
In addition to RFI, crosstalk caused by cross coupling of signals from one
antenna to another, which reveals itself as a static complex bias to a
visibility, which may vary slowly with time. In this case, redundant baselines
would not agree with one another and the $\chi^{2}/\text{DoF}$ would not be
drawn from a mean 1 Gaussian distribution. In other words, the noise in the
deviations from redundancy would not be dominated by the noise model, but would
have an excess due to the crosstalk. This effect would not be as dramatic as
RFI on the $\chi^{2}$, but would be skew the distribution of residuals to not be
centered at 1. In order to account for the crosstalk term, OMNICAL convolved a
10 minute sliding boxcar window over the deviations from redundancy and
subtracted that average off the visibility centered in the window. This
crosstalk removal does not remove the 21 cm signal since it is preserving the
common mode signal from the redundant baselines. For our fiducial power spectrum
baselines, the crosstalk, or additive offset, term is averaged over 51
baselines before being subtracted from each redundant visibility in that
...
\cite{jacobs_et_al2013}. Also plotted are the $1\sigma$ error bars derived from the PyBDSM source extractor and averaged over the multiple snapshots used after being weighted by the beam-squared.
...
the mode right outside the horizon is $.0000002\%$. We apply the latter
...
The final step is to average the entire season into local sidereal time (LST)
with bins of width 43 seconds, a bin size that matches the fringe-rate filter
applied in the compression step. The full season was 135 days long; of these 124
...
%series over 134 days for baseline 1\_4 at 150 MHz () averaged over 5 hours in
...
MHz. However this estimate is much more consistent with the results derived in
...
calcutation of the system temperature, where we are looking in the cold patch, away from the galaxy. We note that at 7 hours LST, there is an increase in the system temperature due to the rising of the galactic plane as seen in figure \ref{fig:coverage}
...
method to our data, and close by discussing the effects of using an empirically estimated 
...
distribution on the sky, $\expval{}$ denotes the ensemble average, and
$\delta^{D}$ is the Dirac delta function. 

XXX: need to check all transposes and convert them to daggers.
...
is simply due to the fact that both contributions have positive \emph{power}. In our analysis, we do not attempt any bias subtraction
...
thus no need to explicitly model and subtract any noise bias, and with our decision to leave
the residual foreground bias in the data, the $b_\alpha$ term no longer appears in our estimator.
...
unity. As long as $\mathbf{M}$ is chosen to satisfy this criterion, the resulting bandpower estimates
...
leads to the smallest possible error bars, [XXX: cite Max's paper ] at the expense of broader
...
array. In our analysis, we group the baselines into several groups of redundant baselines, 
in the sense that within each group there are multiple copies of the same baseline. In the
description that follows, we first estimate the power spectrum separately for each group described in Section \ref{sec:observations}.
...
for every instant in the LST-binned data before averaging over the time-steps within the LST-binned day. XXX: cite fringe-rate paper.

For the next portion of our discussion, consider only the data within a single redundant group. Within each group there is not only multiple identical copies of the same baseline, but in addition (as discussed in
...
\right), 
\end{equation}
where $V^{ri} (\nu, t)$ is the visibility at frequency $\nu$ at time $t$. Each data vector is essentially
a visibility spectrum spanning $10\,\textrm{MHz}$ of bandwidth centered on $151.5\,\textrm{MHz}$,
which amounts to 20 channels, and thus a vector consisting of $20$ elements.
%
...
$\x\x^\dagger$, namely $\mathbf{C} = \expval{\x\x^{t}}$. Unfortunately, in our case the
covariance is difficult to model from first principles, and thus we must resort to an
empirically estimated $\mathbf{C}$. We make this estimation by taking the time average
of the quantity $\x\x^{t}$ over 8.5 hours of LST, estimating a different covariance matrix
for each baseline and for odd versus even days. While an empirical determination of the covariance is advantageous
in that it captures features in the data that are difficult to model from first principles, it
...
that might arise from foregrounds). Note that the inverse covariance weighting employed
here differs from that in P14, in that P14 modeled and included covariances between
different baselines, whereas in our current treatment, 
...
relatively noisy form of the eigenspectra in Figure \ref{fig:eigs}). The final $\mathbf{z}^{ri}$ vectors
...
[XXX: need to be clearer about how there are three different baselines plotted here]
} \label{fig:inv_cov}
\end{figure*}
...
With intuition established for the behavior of $\mathbf{C}^{-1}$, we may compress our
data slightly by averaging baselines together the $\mathbf{z}^{ri}$ into five different sets. That is,
we form
...
In order to implement Equation $\label{eqn:presum_qalpha}$, it is necessary to derive a form for
...
\end{equation}
where $V_{i}(t,\tau)$ is the delay spectrum of baseline visibilities [XXX: is this the first time
we use the term ``delay spectrum"? If so, we need to , $X$ and
...
P14), and $B$ is the bandwidth. [XXX: didn't define wavelength or Boltzmann's constant]
...
before squaring and multiplying by a scalar. Thus, $\mathbf{Q}_\alpha$ role in Equation \eqref{eqn:presum_qalpha} is to perform a frequency Fourier transform on each copy of
$\mathbf{z}$. The result is a separable matrix of the form $\Q_{\alpha} =
...
[XXX: I can make this more rigorous, but it'll require a few more lines of math. Zaki, your call.]. This
separable form holds only for short baselines, where the chromaticity of an interferometer
can be neglected over narrow bandwidths [XXX: cite wedge paper 1]. Fortunately, as discussed
above [XXX: right?], only short baselines are used in our analysis here, and we may thus
write
\begin{equation}
    \qhat_{\alpha} =
...
the Fisher matrix ,such that $\mathbf{F} \equiv \mathbf{L}\mathbf{L}^{\dagger}$, where
...
but not vice versa. Since foregrounds preferentially appear at low $k$, this mitigates leakage
of foregrounds into the EoR window. [XXX: have we used this terminology elsewhere in the paper?] Additionally, our placement of the highest $k$ bin as the third element in our reordering of 
...
that our final data points are uncorrelated with one another. This stands in contrasts to the power-spectrum estimator used in P14, where the Blackmann-Harris taper function induced correlated errors
...
by the typical loss for each stage, except for specific modes the maximum
signal loss applies. These modes are uniformly within the horizon limits or (in the case of
delay filtering) immediately adjacent to the horizon limit.
%XXX LST binning is one stage labeled as lossy in figure that isn't addressed in this table
...
%Using 100 bootstraps. Correction factors are
...
We find a $2\sigma$ upper limit of 390 $mK^{2}$ by fitting a
...
presented in a companion paper \citep{pober_et_al2015}.  In that paper,
...
$\tilde{A}_\tau$ in equation \ref{eqn:delay_transform}.
