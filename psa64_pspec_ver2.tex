...
estimates, we elect to compute the median, rather than the mean. One can see the behavior 
of both statistics in Figure \ref{fig:pspec_variance}, where we
show the absolute value of the mean and median $\Delta^2 (k)$ integrate down as more and more samples in time and
more and more baselines (more ``modes") are included. Some $k$ bins integrate down in the steady fashion
consistent with a noise-dominated power spectrum. Others bottom out on some signal (presumably
some systematic such as foregrounds), while a small handful have the peculiar property of
suddenly decreasing as more modes are included. In any case, one sees that the mean and the median
are quite similar, but we expect the median to provide a
...
\caption{
[XXX state that it is the absolute value plotted.Done. Check off?]
[XXX add in what the modes are: bls X time modes]
Cumulative mean (left) and median (right), as a function of number of modes (defined
as different baselines or different LSTs),
of the absolute value of the power spectrum band power for
