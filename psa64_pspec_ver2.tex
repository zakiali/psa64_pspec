...
configuration (\cite{parsons_et_al2014a}). This limit was acheived by using the
...
Additionally, we employ optimal quadratic estimators to make our power spectrum
estimates.
...
Here, we describe the chief features of the data set used in this analysis.
We use data taken while PAPER consisted of 64 dual polarization antennas
deployed in the Fall of 2012, arranged in a maximally
redundant configuration as seen in Figure \ref{fig:antenna_positions}. The
correlator process all 64 dual polarization antennas with 100 MHz of 
instantaneous bandwidth ranging from 100-200MHz channelized to 1024
channels. We write data for both X and Y polarization as well as their cross
products, but this analysis focusses on the XX and YY polarizations. 

We rely on all 2016 baselines for the calibration procedure to, but only use a
subset of the baselines for the power spectrum analysis. This subset encompases
3 separation types; the shortest spaceing east-west baselines (e.g. 41-47) and
those of the same type except slanted up and down one row (e.g. 10-41 and 10-58,
respectively). Baselines in each of the 3 separation types (or redundant groups)
are instantaneously redundant within the group and therefore measure the same
fourier modes on the sky. 
...
siderial time (LST), a range of 0:00 to 10:00 hours. This range corresponds to
...
sidereal time and apply a frine rate filter to optimally combine the
...
PAPER's highly redundant configuration lends itself to
use the redunancy in baselines for a relative calibration. Multiple baselines of
the same length and orientation measure the same sky signal. 
On this basis, all of the differences between baselines of a
given unique baseline type are attributed to differences in signal chain, which
include variations in antennas, cables, and receivers, and are calibrated
out. This leads to a relative calibration solutions that relate the antennas
within the array. Redundant calibration does not give absolute solutions, and
therefore we are not able to phase to a source, for example, after applying
these solutions. Redundant calibration inherently preserves common mode signal between
...
does not set an absolute phase or gain scales due to the fact that it only
...
the previous homegrown routine used in \cite{parsons_et_al2014a}. Figure
\ref{fig:omniview} shows complex visibilites after complex gain solutions
derived from Omnical were applied to the uncalibrated visibilities. This
comparison shows the merrit of using Omnical.
...
calibration (selfcal) method for radio interferometers to solve for
...
As before, redundant calibration does not set a global flux scale to accurately
measure the brightness of the sky. To set the absolute flux scale, we use Pictor
A as our calibrator source using the value derived in \cite{jacobs_et_al2013}.
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{plots/pic_spec.pdf}
\caption{Pictor spectrum.}
\label{fig:pic_spec}
\end{figure}

XXX Need to include discussion of signal loss due to fitting the polynomial. XXX
...
To optimally combine the time data we can cater our fringe rate filter to
...
bins by the beam, we can get a net increase in sensitivity. This is due to the
fact that all fringe rate bins roughly have the same noise in them, but the
amount of signal varies. Upweighting the bins with higher signal relative to
those with less signal, gives us a net increase in signal-to-noise, even though
we are getting rid of signal from some of the bins. 

We implement the optimal fringe rate filter by calculating the fringe rates at
every point on the sky and weighting each bin by the beam of a given baseline. It is
worth noting that fringe-rate filtering is baseline dependent and hence is
implemented on a per baseline basis. We weight the fringe rate bins on the sky
by the beam pattern at the same point on the sky. In order to obtain a smoothe
filter in the fringe rate domain, we fit a gaussian with a tanh tail to
this filter as shown in figure \ref{fig:fringe_rate_cut}. We then fourier
transform this to obtain the time domain filter, multiply by a blackman-harris
window function to damp the tails of the filter, and finally convolve with the
visibilites to effect time domain integration.
...
leaving us with only ~20 independent measurements over the lst range
we are considering. This reduction in modes leads to a noisy estimate of the
covaraince matrix. This leads to a broader problem in this type of analysis: the
fact that we estimate our covariances from the data and do not know the true
covariance matrix itself, leads to signal loss. Knowing the true covaraiance of
a data set requires complete knowledge of the instrument and the sky. 
...
\begin{figure}[h!]\centering
\includegraphics[width=\columnwidth, height=\columnwidth]{plots/inv_cov.png}
...
averageing time samples for roughly 30 minutes. Over the 10 hour LST range used in this
analysis, this corresponds to ~20 independent modes on the sky. This is a
fundamental limit of the amount of information we have. 
...
for the frequency bins (used in this paper?). The signal injected into the
...
\section{Comparison and Improvements From PSA-32}
...
\begin{figure}[h!]\centering
\includegraphics[width=\columnwidth, height=.8\columnwidth]{plots/pk_k3pk_nomni.png}
...
\begin{figure}[h!]\centering
\includegraphics[width=\columnwidth, height=.8\columnwidth]{plots/pk_k3pk_nofrf.png}
