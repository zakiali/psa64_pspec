...
\newcommand{\mKlimit}{(19.7 mK)$^2$}
\newcommand{\mKsqlimit}{390 mK$^2$}
...
review the optimal quadratic estimator formalism, with an emphasis on our power
...
covariance matrices.  For a more detailed examination of the application
of optimal quadratic estimators to 21cm cosmology, we refer the reader to
\citet{liu_et_al2014a} and \citet{liu_et_al2014b}.
...
%
The normalization matrix is the analyst's choice, and is the last chance to
...
$\mathbf{M}\propto\mathbf{F}^{-1}$ implying that $\mathbf{W} = \mathbf{I}$. This
window function is a delta function such that every band power does not
contain leakage from other band powers, and therefore the estimates are well
localized and contain power from only that bin. However, this has the downside
that the error bars are large.  On the other hand, choosing the identity
normalization such that $\mathbf{M}\propto\mathbf{I}$ implies $\mathbf{W}=\mathbf{F}$,
leading to the smallest possible error bars, with the consequence that every
band power is coupled to every other band power \citep{liu_tegmark2011}.
...
\caption{Posterior distribution of power spectrum amplitude for a flat $\Delta^{2}(k)$
