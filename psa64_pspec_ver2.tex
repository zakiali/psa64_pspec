...
\newcommand{\mKlimit}{(19.7 mK)$^2$} % XXX put in final number
\newcommand{\mKsqlimit}{390 mK$^2$} % XXX put in final number
...
Jasper Grobbelaar\altaffilmark{9},
Jasper Horrell\altaffilmark{9},
Daniel C. Jacobs\altaffilmark{14}, 
Pat Klima\altaffilmark{7},
David H. E. MacMahon\altaffilmark{2},
Matthys Maree\altaffilmark{9},
David F. Moore\altaffilmark{5},
Nima Razavi\altaffilmark{13},
Irina I. Stefan\altaffilmark{13},
...
\altaffiltext{14}{School of Earth and Space Exploration, Arizona State U., Tempe AZ}
...
this paper \citep{pober_et_al2015}, this upper limit supports and extends
...
[XXX negligible signal loss]
[XXX link signal loss to table]
}
\label{fig:flowchart}
\end{figure*}
...
RFI at the 6$\sigma$ level, applying low-pass delay and fringe-rate filters that limit signal variation
...
After the wideband delay filter, we remove a second layer of RFI and crosstalk
 ...
 a filter which flags values $3\sigma$ above the median using a variance calculated in a localized time and frequency window.  Crosstalk, which is an overall complex offset to the visibility, due to small couplings between independent analog signal chains, is removed by subtracting a ten minute long average from each baseline. Normally, hour long averages are needed for foreground contained data
...
[XXX above statement is incorrect]
...
\caption{
[XXX update filter shape, which data to use?]
...
\caption{
[XXX change y label to fractional variance]
[XXX update caption to describe how to interpret stability]
[XXX is this figure linked to in text?]
[XXXVariance relative to the mean. Fractional variance.]
Power spectrum of 135 days of time-series data contributing to a single LST bin,
...
done in \citet{dillon_et_al2013a} and \citet{liu_tegmark2011}.  Here we briefly review the
...
that \begin{equation} 
 \boldsymbol   \Sigma = \mathbf{D} \mathbf{L}^{-1}\mathbf{L}\mathbf{L}^{\dagger}\mathbf{L}^{-\dagger} \mathbf{D}
           = \mathbf{D}^2.
\end{equation}
[XXX define F somewhere]
[XXX put F figure before W figure]
[XXX explain one last time the choice of M=Linv]
...
lossless estimators of the power spectrum [XXX: cite Tegmark 97]. However,
...
\begin{itemize}
\item Visibilities are averaged into five baseline groups after inverse covariance weighting (see Equation \ref{eqn:presum_oqe})
...
and averaged together as though they were the original data. This forms a single estimate (a ``bootstrap") of $P(\mathbf{k})$. Repeating this exercise over and over again with different
...
$P(\mathbf{k})$, we add a binary parameter to our random draws that specifies whether we
draw from $+k$ or $-k$.

In our analysis, we compute a total of 400 bootstraps. In extracting our final power spectrum
estimates, we elect to compute the median, rather than the mean. The median provides a
better rejection of outliers, such as those that might arise from residual foreground power. In doing
so, however, we must take into account the fact that our power spectrum estimates may be
drawn from a distribution where the mean and median do not coincide. For example, if our power spectrum estimate were
dominated by the true cosmological power spectrum, the scatter in the measured values would
be Rayleigh distributed due to cosmic variance. For a Rayleigh distribution, the median is $\ln2 \sim 0.69$
times the mean. On the other hand, if our power spectrum estimation is dominated by instrumental
noise, data points will be peaked and symmetric about zero (thanks to our use of cross- rather than auto-power spectra), and thus the mean will coincide with the median. To be conservative, we apply
the $\ln2$ correction factor to all our power spectrum estimates.
...
\caption{
[XXX change thermal noise power spectrum to 2-sigma upper limit in the absence of a celestial signal]
...
\caption{
[XXX update with non-xtalk removed data]
...
\caption{
[XXX update to remove dashed curve]
