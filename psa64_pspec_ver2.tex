...
The Fisher matrix quantifies
the amount of information about each $k$-mode. The
Fisher matrix is shown in Figure \ref{fig:fisher}; the diagonal quantifies how constraining measurements
are on the amplitude of 
every $k_{\parallel}$ mode individually, and off-diagonal how well modes are differentiated from
one another. The Fisher matrix is derived
using all of the baselines, contains of all of the
information associated with the estimate of $P(k)$.  However, modes can be re-weighted using
$\mathbf{M}$ to trade between vertical and horizontal errors.
