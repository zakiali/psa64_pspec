...
\caption{
Fringe rate transform of a 30 m east-west baseline at 159 MHz before (blue) and
after optimal fringe rate filtering (red). The fringe rate filter (green) is
scaled so that that the ratio of the maximum amplitude in the pre and post
fringe rate filtered data is preserved.
...
\caption{
Application of inverse covariance weighting to visibilities from 30 m east-west
baselines over 146-156 MHz frequecy range and a LST range of 0h00 -- 8h30 used
in the quadratic estimator of the power spectrum. From top to bottom, each row
represents (1) the measured visibilities, (2) the covariance matrix derived from
the visibility, and (3) visibilities after the application of inverse covariance
weighting.  The left three columns correspond to 3 different baselines of the
same redundant group prior to delay filtering and the right three columns
correspond to the same three baselines after the application of the delay
filter.}
\label{fig:inv_cov}
\end{figure*}
...
\caption{Eigenspectrum of empirically estimated covariance matrices (left panel)
from visibilities prior to foreground filtering (blue) and after the application
of the delay filter(green). The right panel shows the the four strongest
eigenmodes of the covariance matrix estimated prior to foreground filtering
(top) and after foreground filtering (bottom).}
...
\caption{The Fisher information matrix for the quadratic estimator described in
equation \ref{eqn:presum_qalpha}, as a function of $k_{\parallel}$. The
color scale represents strength (from blue to red) of the covariance between
$k_{\parallel}$ modes.}
...
\caption{Cumulative mean (left) and median (right) of band powers for a
given $k$-mode in the power spectrum shown in figure \ref{fig:final_pspec}.
Color represents the absolute value of $k$-mode being integrated, from small to
large modes represented from blue to green, respectively.}
...
\begin{table}[htdp]
\caption{TABULATED SIGNAL LOSS THROUGH VARIOUS ANALYSIS STAGES}
\begin{center}
...
\caption{Real part of 
complex visibilities for the strictly east-west 30 m redundant group of
baselines at XXX MHz, for all integrations. The
color scale corresponds to the number of samples that went into the specific
LST/visibility bin. For the densest regions, the spread in visiblities is
approximately 100 Jy.}
...
\caption{The power spectrum of the time series that went into LST averaging at
150 MHz for the full 135 day observation described in section
\ref{sec:observations} using a single 30 m east-west baseline. The power
spectrum was averaged of a 5 hour LST range to beat down noise.}
...
\caption{System temperature as a function of frequency plotted for multiple 1
hour LST ranges. For the majority of the LST range used in the analaysis, we
find that $T_{sys}=500K$ at 160 MHz.}
...
power spectrum,  we can begin to put constraints on the mean brightness
temperature of the 21 cm line. We can begin to provide contstraints on the
neutral fraction and heating of the IGM at a redshift of 8.4. In a companion
paper \citep{pober_et_al2015a}, we conduct a thorough analysis of the
constraints we can put on the IGM using reionation simulations with 21cmFAST
[XXXcite], a 21 cm simulation code. As a compliment to the more thourough
analysis, we focus on a simpler model, as discussed in P14 and P12a that can put
some constrains on the mean brightness temperature for the limits presented in
figure \ref{fig:final_pspec}.
...
\begin{equation}\label{eqn:final_tb}
    \delta{T_{b}} \approx
25x_{\text{\tiny{HI}}}\Big(\frac{1+z}{10}\Big)^{1/2}(1+\delta) \xi \,\text{mK},
\end{equation}
where $\xi =  \Big[1 - \frac{T_{\gamma}(z)}{T_{s}}\Big]$. 

If the $T_{s}$ is greater than $T_{\gamma}$, we get the the 21 cm signal is in
emission with respect to the CMB. However, if $T_{s}$ is less than $T_{\gamma}$,
equation \ref{eqn:final_tb} is negative and hence the 21 cm is in absorption.
The thermal history of the 21 cm line follows a very predictable and well
established path through cosmic time starting with being coupled to the
$T_{\gamma}$ via compton scattering, and hence there is no detectable 21 cm
signal. As the universe cools adiabatically, the gas temperature decreases as
$(1+z)^{2}$ and colissional coupling puts the spin temperature below the CMB
temperature and the 21 cm line is in absorption. Just before the first sources
turn on to couple the spin temperature back to the spin temperature back to the
gas temperature  via the Wouthuysen-Field effect, collsional coupling becomes
inefficient and there is no detectable 21 cm signal due to radiative coupling
between the gas and the CMB. Once the first sources turn on, the gas temperature
...
from the usual suspects. In this case, the 21 cm  signal will be in absorption.
In this scenario we can take a reasonable rescaling of power spectra to get a
sense of the mean brightness temperature for a simple flat power spectrum model
varying with the neutral fraction, $x_{\text{HI}}$, as presented in P14. Namely,
because we have assumed no heating, we can take the combination
...
\end{equation}
where $T_{0} = 25 \big(\frac{1+z}{10}\big)^{1/2}$ and $\Delta_{i}^{2}(k)$ is a
model power spectra of the ionization fluctuations. The maximum value of the
...
CMB must be in equilibrium until about a redshift of 150
\citep{furlanetto_et_al2006} and hence the maximum value of $\xi$ is $\xi = 1 -
\frac{1+z_{dec}}{1+z} \approx -\frac{150}{1+z}$ for the redshifts under
consideration. Therfore, combining equations \ref{} and \ref{} we get that the
the maximum value of the prefactor in \ref{d2pspec_model} is given by 
\begin{equation}
...
approximates the 21 cm power spectrum as being flat between some $k_{\text{min}}$ and
$k_{\text{max}}$ cutoff values in $k$, 
\begin{equation}\label{eqn:patchy_bound}
    \Delta^{2}_{i}(k) = (x_{\text{HI}} -
x_{\text{HI}}^{2})/\ln{i(k_{\text{max}}/k_{\text{min}})}.
\end{equation}
For this reionzation model, we tune $k_{\text{min}}$ and $k_{\text{max}}$ and
determine the maximum mean brightness temperature above which equation
\ref{eqn:d2pspec_model} becomes inconsisitent with our 21 cm power spectrum
measurements. Figure \ref{fig:patchy_bound} shows the results of these bounds
for the three different neutral fractions $x_{\text{HI}}=\text{.1, .3, and .5}$
...
\caption{
Power spectrum, shown as P($k$) (left) and $\Delta^{2}$ (right), at $z=8.4$ resulting from a 134 day PAPER observation
...
vertical lines at $k\approx{.6\ h\ Mpc^{-1}}$ corresponds to the bounds of the
delay filter described in section \ref{sec:delay_filter}. The theoretical
$2\sigma$ thermal noise power spectrum (dashed cyan) corresponds to the
theoretical minimum error bars consistent $T_{sys}=500K$.
XXX (To be done) The solid cyan curve is the power spectrum of the
foreground contained data. 
...
\caption{The posteriror distribution of power spectrum values in $\Delta^{2}$
assuming a flat power spectrum and gaussian error bars. The green and red
...
\caption{Power spectra with and without various analysis stages applied to data.
Power spectra without optimal fringe rate filtering (dot-dashed) and without
omnical calibration (dashed) are shown along with the final power spectrum
(solid) as shown in figure \ref{fig:final_pspec}. This veritcal dashed line
corresponds to the bounds of the wideband delay filter. }
