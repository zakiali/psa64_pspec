...
In our analysis, $\x$ is the set of visibilities for all redundant
baselines of a single column east-west spacing. The covariance matrix is defined
as the outer product of the data vector with itself, $\mathbf{C} =
\expval{\x^{t}\x}$.  $\mathbf{Q}$ is the derivative of $\mathbf{C}$
with respect to $\alpha$, and $b_{\alpha}$ is a bias to the estimate. The
normaliztion matrix is defined such that
\begin{equation}
...
In our analysis we are constructing an unbiased estimator and therefore do not
need to subtract the bias term in equation \ref{eqn:quad_est}. 
...
Table \ref{table:pspec_sig_loss} tabulates the signal loss associated with our
