...
$0.1<k<0.5\ h\ {\rm Mpc}^{-1}$ at $z=8.4$.  This represents a four-fold improvement
...
500K$ at 160 MHz. [XXX Zaki, how exactly did you get 500?  the number from P14 and J15 was 550K @ 160MHz]
...
$3.0\sigma$ excess at $k\approx{.35}\ h\ {\rm Mpc}^{-1}$, and to a lesser
degree, the $2.1\sigma$ deficit at $k\approx{.39}\ h\ {\rm Mpc}^{-1}$.  The
...
Gaussian statistics.  However, the outlier at $k\approx{.35}\ h\ {\rm Mpc}^{-1}$ is
unlikely to be chance.

We note that an excess centered at this wavemode was also
measured in P14, approximately an order of magnitude higher in amplitude.  This
suggests to us that the excess may be a result of foreground emission that is attenuated
(but not eliminated) in the current analysis.  In examining the effects on the power
spectrum of omitting various stages of analysis (see Figure \ref{fig:pspec_comp}), we
see that the magenta curve corresponding to the omission of fringe-rate filtering
shows the excess perhaps even more clearly. 
This supports the foreground emission theory.  The signal could perhaps be associated with
...
a single upper limit, we fit a flat $\Delta^(k)$ model to measurements
in the range $0.15<k<0.5h\ {\rm Mpc}^{-1}$.  
Figure \ref{fig:final_posterior} shows the posterior distribution of the fit,
assuming Gaussian errors. From this distribution, we determine a $2\sigma$ upper
limit to be $(20.6 {\rm mK})^{2}$. 
which is consistent with zero. [XXX is this still true?]
...
power spectrum over $k=.15 - .5 \rm \,hMpc^{-1}$ (solid black),
assuming Gaussian error bars. The blue and orange
vertical lines correspond to the 1 and 2 $\sigma$ upper limits, respectively,
on the power spectrum. The poseterior distribution excluding the the detection at $k=.35\rm \,hMpc^{-1}$ (dashed black) is shown for comparison.
}
...
transition by the $2\sigma$ upper limits shown in figure \ref{fig:final_pspec}
at a redshift of 8.4 assuming the pathchy reionization model given by equation
\ref{eqn:d2pspec_model} and \ref{eqn:patchy_bound}. Color indicates $T_{s}$ (in K) for a combination of $k_{min}$ and $k_{max}$.} 
