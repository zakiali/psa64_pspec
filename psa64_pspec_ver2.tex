...
variance weighting. For an infinite sample of random white noise, the covariance
matrix would be diagoanl and applying the inverse covariance would be identical
to inverse variance weighting. The diagonal of this inverse covariance matrix
are the eigenvalues and correspond to the the eigenmodes. Therefore, the most
downweighted modes correspond to the largest eigenvalues of $\C$. Similarily,
applying the inverse covariance to the baseline data vector, $\x$, the strongest
modes get down weighted. 
...
The strongest modes for delay filtered data look like noise and lack coherent
structure.
...
The data vector used in our quadratic estimator is arranged such that redundant
baselines and frequency channels are serialized into the the vector
$\mathbf{x}$, where the channels are cycled through more frequenctly then
baselines. The columns of our vector consist of the time samples for each of the
vectors. As stated above, the covariance matrix is given by $\mathbf{C} =
\expval{\x\x^{\dagger}}$. Hence the ensemble average of the covariance occurs
over the time axis, where because of fringe rate filtering, the number of
independent modes is ~20. 
