...
\caption
{
Flow of data through the power-spectrum analysis steps described in in the paper. Data flow is indicated b the solid black lines and red lines indicate injected simulated data to measure the signal loss and pipeline efficacy. Yellow boxes are those which do not have signal loss associated with them and are not used in the signal loss measurements.
This is currently just place holder figure.XXX.
}
\label{fig:flowchart}
\end{figure*}
...
\caption{
Each panel is a scatter plot of complex visibilites, for every baseline in the
array, in the complex plane at 159 MHz for a single integration. Since
redundant baselines should measure the same sky we expect that there is
clustering between visibilities of baselines in a redundant group. Each symbol
and color combination corresponds to a redundant group of baselines. The left
pannel shows the the raw data after a rough calibration has been applied to the
data. There is clustering between redundant baselines, but there is still some
thermal noise in the measurement that is do to the imprecise nature of the
rough calibration and cross talk between baselines. The right panel shows the
same data after using calibration parameters derived from Omnical. The reundant
baselines come into better alignment after those calibration solutions are
applied, nodding to the power of using Omnical.}
\label{fig:omniview}
\end{figure*}
...
\caption{This is a waterfall plot for a days $\chi^{2}$ distribution from our
fitting of the visibilities. The $\chi^{2}$ here is calculated for every
frequency bin and integration in our data set, 49kHz and 42.9 seconds,
respectively. The $\chi^{2}$ demnstrates the stability of the instrument and is
a useful tool for flagging bad data, corresponding to very large $\chi^{2}$
values.}
\label{fig:chi2}
\end{figure}
...
\caption{
A field image phased to Pictor A and including Fornax A using the maximally
redundant array configuration data set used in the analaysis presented in this
paper. The elongated north-south stripes are due to the lack of north-south
baseline used here. In addition grating lobes due to the antenna configuration
can be seen (?). The location of Pictor A and Fornax A agree with their catalog
values of ...}
...
\caption{
The model Pictor A spectrum as given in \citet{jacobs_et_al2013}
(black) along with the measured Pictor A spectrum (blue), Stokes I, used in
setting our flux scale. The spectrum was calculated by imaging our maximally
redundant configuration array to Pictor A for every frequency, and reading off
the flux. The ripples in the measured spectra are due to beam sidelobes from
interfereing sources, specfically Fornax A.}
\label{fig:pic_spec}
\end{figure}
...
observing season with log amplitude and phase on the top and bottom rows,
respectively. LST range is from -2.2 to 17.3 hours and a frequency range from
100 to 200 MHz. Dashed lines indicate the range of LST used in the power
spectrum analysis (0 - 8:30 hours). Foreground averaged data (left) is fringe
rate filtered with the optimal fringe rate filter (second left) and signal is
retained, but signal low in the beam is down weighted. Visibilities have a
wideband delay filter applied to them to suppress foreground signal (middle)
which achieves 2 orders of magnitude signal suppression in Jansky's. After
applying the optimal fringe rate filter to the foreground filtered data (second
right), we achieve 3 orders of magnitude suppression of foregrounds, and reach
the longest possible coherent time integration. Finally, averaging all of the
redundant 30 m East-West baseline, effecting 4 orders of magnitude below the
...
\begin{equation}\label{eqn:patchy_bounds}
