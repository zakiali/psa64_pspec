...
\end{equation}
where $V_{i}(t,\tau)$ is the delay spectrum of baseline visibilities given by Equation \eqref{eqn:delay_transform}, $X$ and
...
in Section \ref{sec:observations}, only short baselines are used in our power spectrum analysis. 
The approximate separability of $\Q_\alpha$ therefore applies, and we may write
\begin{equation}
    \qhat_{\alpha} =
...
$P(\mathbf{k})$, we bin power falling in $+k$ and $-k$, and so we add a binary parameter to our random draws that specifies whether to draw from the positive $k$ bin or the negative one.
