...
\textbf{with each having 56, 49, and 49 baselines, respectively}.
...
%       --Reiterate that redundant cal does not solve for the overall  and
%         phase parameters. 
...
distribution of visibilities around a patch of sky, a LST bin for a
...
varies and is determined by how the primary beam responds to the sky.
...
\textbf{Generally, a fringe-rate filter integrates visibilities in time. For a 
...
number of independent samples can lead to signal loss in the optimal quadratic estimator. In order to counteract the signal loss, we degrade the optimal fringe-rate filter, as shown in Figure \ref{fig:fr_preserved_signal}, to have an effective integration time of 1886 seconds, thus increasing the number of independent modes to 2 per hour. Even though the fringe rate filter is now sub-optimal, it holds a big improvement over the boxcar fringe rate weighting as used in P14. As documented in Table \ref{tbl:sigloss}, the correction factor for the associated signal loss of the filter we have chosen is \textbf{1.39}.}
...
\textbf{The Hydrogen Epoch of Reionization Array (HERA) and the low frequency Square Kilometre Array (SKA-Low) are next generation experiments that aim to make a significant detection of the 21 cm power spectrum and even begin characterizing it. SKA-Low, the successor to LOFAR and MWA,  has secured pre-construction funding for a build out in western Australia. HERA, on the other hand, was recently granted funding for its first phase under the National Science Foundation's {\it Mid-Scale Innovations Program}. It has been
designed to have the maximum sensitivity possible for an epoch of reionization array consisting of a large collecting area and short baselines. HERA uses a close packing of 14-m diameter 
