...
for inject in glob.glob('data/sigloss/inject_*'):
    ...
    pCs_err.append(n.std(pC_avg))
    pIs.append(n.average(pI_avg))
    pIs_err.append(n.std(pI_avg))
...
p.subplot(gs[2])
#p.loglog(pIs, pCs, 'k.')
p.errorbar(pIs, pCs, xerr=pIs_err, yerr=pCs_err, capsize=0, fmt='k.')
...
p.errorbar(pIs/pCs - 1, pCs, xerr=pIs_err/pCs, yerr=pCs_err, fmt='k.', capsize=0)
ax3.set_xscale('log')
ax3.set_yscale('log')
p.ylim(pklo, pkhi)
p.xlim(1e-3,20)
...
p.errorbar(pIs, pCs/pIs, xerr=pIs_err, yerr=pCs_err/pIs, fmt='k.', capsize=0)
ax0.set_xscale('log')
p.xlim(pklo, pkhi)
#p.ylim(3e-2,3)
p.ylim(0,1.1)
