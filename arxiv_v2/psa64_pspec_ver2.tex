...
\documentclass[twocolumn,numberedappendix]{emulateapj} \shorttitle{New Limits on the $21\,\textrm{cm}$ Power Spectrum at $z=8.4$}
...
\title{PAPER-64 Constraints on Reionization: the  $21\,\textrm{cm}$ Power Spectrum at $z=8.4$}
...
the newly funded Hydrogen Epoch of Reionization Array.  
\end{abstract}
...
the $21\,\textrm{cm}$ ``spin-flip" transition of neutral hydrogen at high redshifts.
...
each observed frequency with a particular emission time (or distance), enabling $21\,\textrm{cm}$ experiments 
...
in the near term, $21\,\textrm{cm}$ cosmology experiments are focusing on statistical measures
of the signal.
...
There are two complementary experimental approaches to accessing $21\,\textrm{cm}$ emission from
...
seek to measure the
mean brightness temperature of $21\,\textrm{cm}$ relative to the CMB background. These experiments
typically rely on auto-correlations from a small number of dipole elements to access
the sky-averaged $21\,\textrm{cm}$ signal, although recent work is showing
that interferometric cross-correlations may also be used to access the signal
\citep{vedantham_et_al2015,presley_et_al2015}.
In contrast, experiments targeting statistical power-spectral measurements of the $21\,\textrm{cm}$
...
now relies to solve for the majority of the internal instrumental degrees of freedom (dof).  The efficacy of
this approach was demonstrated with 
data from a 32-antenna deployment of PAPER, which achieved an upper 
limit on the $21\,\textrm{cm}$ power spectrum $\Delta^2(k)\leq (41\,\textrm{mK})^{2}$ at 
...
\includegraphics[width=\columnwidth]{antenna_positions.png}
\caption{
...
separations of zero (black; e.g. 49-41, 41-47, 10-3, \ldots)
one in the northward direction (orange; e.g. 10-41, 3-47, 9-3, \ldots) or
one in the southward direction (blue; e.g. 49-3, 41-25, 10-58, \ldots).
...
%\end{figure*}
\begin{figure*}\centering
\includegraphics[width=2\columnwidth]{coverage.png}
...
described in Section \ref{sec:observations}.  The map is centered at 6:00 hours in RA.
}\label{fig:coverage}
\end{figure*}
...
down to the horizon with a FWHM of $60^{\circ}$, is summarized in \citet{parsons_et_al2010}
...
strictly east-west baselines between adjacent columns (e.g. 49-41, black 
...
those whose eastern element is one row down (e.g. 49-3, blue in Figure \ref{fig:antenna_positions}).
These baseline groups consist of 56, 49, and 49 baselines, respectively.
...
\begin{figure*}
%\includegraphics[width=2\columnwidth]{plots/data_flow_chart.png}
\includegraphics[width=2\columnwidth,trim=0cm 7cm 5cm 2cm,clip]{data_flow_chart.pdf}
\caption
{
The stages of power-spectrum analysis. Black lines indicate data flow; red lines indicate
Monte Carlo simulations used to 
...
Foreground contamination and signal sensitivity represent the two major concerns for $21\,\textrm{cm}$
...
In the low-frequency radio band (50--200 MHz) where $21\,\textrm{cm}$ reionization
...
imitate line of sight Fourier modes over cosmological volumes (P12b; \citealt{,morales_et_al2006a,bowman_et_al2009}).
...
\citep{liu_et_al2008,bowman_et_al2008,harker_et_al2009,sullivan_et_al2012,chapman_et_al2013}.
...
an interferometric baseline.  So far, PAPER has had the most success in exploiting this bound (P14; \citealt{jacobs_et_al2014}). 
...
for measurements of the $21\,\textrm{cm}$ reionization signal in these ranges.
...
per-antenna internal dof in the array.  In the second stage (Section \ref{sec:abscal}), standard self-calibration is used 
...
Finally, we use an
OQE (Section \ref{sec:oqe}) to make our estimate of the $21\,\textrm{cm}$ power spectrum.
...
way to solve for internal dof in radio interferometric measurements without simultaneously
...
\begin{figure*}
\centering
\includegraphics[width=1.5\columnwidth]{omniview_64.png}
\caption{
...
quality of our data. The number of dof, as defined in \citealt{zheng_et_al2014}, is given by 
\begin{align}
...
crosstalk removal preserves signals common to redundant baseline groups (such as the $21\,\textrm{cm}$ signal).
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{picimg_cs.png}
\caption{
...
\includegraphics[width=\columnwidth]{picspec.png}
\caption{
...
\begin{figure*}
\centering
\includegraphics[width=2\columnwidth]{waterfalls_labeled.png}
\caption{
...
constant fringe rate correspond to rings in R.A. and decl., where the east--west
...
bins by the rms of the beam response, we can get a net increase in sensitivity.  
...
decl. of the array. For PAPER, this filter roughly doubles the
sensitivity of our measurements.  

Generally, a fringe-rate filter integrates visibilities in time. For a 
...
\begin{equation}\label{eqn:frf_inttime}
    t_{\text{int,after}} = t_{\text{int,before}}\frac{\int{\sigma_{f}^{2}df}}{\int{\sigma_{f}^{2}f_{\text{fr}}^{2}df}},
\end{equation}
where $t_{\text{int,before}}$ is the integration time before filtering, $\sigma_{f}$ denotes the noise variance in fringe rate space and the integral is taken over all possible fringe rates for a given baseline and frequency. As discussed in \citet{parsons_et_al2015}, the
signal re-weighting  associated with this fringe-rate filter can be interpreted as a modification
to the shape of the primary beam.

For the fiducial baseline at 151 MHz, the integration time, as given in equation \eqref{eqn:frf_inttime}, associated with an optimal fringe rate filter is $3430\, \textrm{s}$. The number of statistically independent samples on the sky decreases from 83 to 1 sample per hour. As discussed in section \ref{sec:sigloss}, empirically estimating a covariance matrix with a small 
number of independent samples can lead to signal loss in the OQE. In order to counteract the signal loss, we degrade the optimal fringe-rate filter, as shown in Figure \ref{fig:fr_preserved_signal}, to have an effective integration time of 1886 s, increasing the number of independent modes to 2 per hour. The fringe rate filter is now sub-optimal, but is still an improvement on the boxcar weighting as used in P14. As documented in Table \ref{tbl:sigloss}, the correction factor for the associated signal loss of the filter we have chosen is 1.39.
...
fringe rates below $0.2\,\textrm{mHz}$. This removes 
...
\begin{figure}\centering
\includegraphics[width=\columnwidth]{fringe_rate_filter.png}
\caption{
...
\begin{figure*}
\centering
\includegraphics[width=2.3\columnwidth]{density.png}
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{stability.png}
\caption{
...
In order to build sensitivity to the $21\,\textrm{cm}$ reionization signal, it is critical
...
$\Tsys\sim$500K.  We get
more samples per LST bin in the range of 2--10 hr due to our observing
...
excess on two-day timescales caused by the changing alignment of the $42.9\,\textrm{s}$
...
with the results derived in \citep{moore_et_al2015}, where $\Tsys=505\,\textrm{K}$ at 164
...
\begin{figure}\centering
\includegraphics[width=\columnwidth]{tsys.png}
...
review the OQE formalism, followed by a walk-through of our 
...
\subsection{Review of OQEs}
We use the OQE method to estimate our power spectrum as
...
OQE formalism with an emphasis on our application
to data, which draws strongly from the aforementioned works, but also relies on empirical
techniques similar to those used in P14. The end goal of this analysis is to estimate the $21\,\textrm{cm}$
...
$\delta^{\text{D}}$ is the Dirac delta function. 
...
\end{equation}
which will be needed to form the OQE of our power spectrum.
...
\subsection{Application of OQE}
\label{sec:oqe_app}
% A walk through of the application to the data.

Here we describe the specifics of our application of the OQE
formalism to measure the power spectrum. Doing so requires defining
...
of the quantity $\x\x^\dagger$ over 8.5 hr of LST, estimating a different covariance matrix
...
\begin{figure*}\centering
\includegraphics[width=2\columnwidth]{inv_cov.png}
\caption{
...
\begin{figure}\centering
\includegraphics[width=\columnwidth]{eig.png}
\caption{
...
\begin{figure}
\centering
\includegraphics[width=\columnwidth]{sigloss.png}
\caption{
...
final estimate of the $21\,\textrm{cm}$ power spectrum (Figure \ref{fig:final_pspec}), we
...
\begin{figure*}\centering
\includegraphics[width=1.8\columnwidth]{pspec_variance.png}
\caption{
...
When estimating our power spectra via OQEs, we generate
...
\begin{figure*}\centering
\includegraphics[width=2\columnwidth]{pk_k3pk.png}
\caption{
...
limit (P14) at $z=7.7$ (green). The magenta curve shows a predicted model $21\,\textrm{cm}$ power
...
\begin{figure*}\centering
\includegraphics[width=2\columnwidth]{pspec_comparison.png}
\caption{
...
culprit is polarization leakage \citep{jelic_et_al2010,jelic_et_al2014,moore_et_al2013}, although further
...
\begin{figure}\centering
\includegraphics[width=\columnwidth]{flat_k3pk_posterior.png}
\caption{
...
for the heating of the IGM prior to the completion of reionization.

\subsection{Spin Temperature Constraints}

\begin{figure*}\centering
\includegraphics[width=2\columnwidth]{ts_patchy_bound.png}
...
If the spin temperature is larger than $\Tcmb$, we get the $21\,\textrm{cm}$ signal in
...
implied by the $2\sigma$ $21\,\textrm{cm}$ power
...
in redundant measurements to an OQE (with carefully calibrated signal
...
We present new upper limits on the $21\,\textrm{cm}$ reionization power spectrum at $z=8.4$,
...
improved redundant calibration with OMNICAL, and with an OQE
that, while not perfectly lossless, is more adept at down-weighting residual foregrounds.
...
The Hydrogen Epoch of Reionization Array (HERA) and the low frequency Square Kilometre Array (SKA-Low) are next generation experiments that aim to make significant detections of the $21\,\textrm{cm}$ power spectrum and begin characterizing it. SKA-Low has secured pre-construction funding for a facility in western Australia. HERA was recently granted funding for its first phase under the National Science Foundation's {\it Mid-Scale Innovations Program}. 
...
1129258, and 1125558).  A.R.P, J.C.P, and D.C.J would like to acknowledge NSF support
(awards 1352519, 1302774, and 1401708, respectively).
J.E.A would like to acknowledge a generous grant from the Mount Cuba Astronomical Association for
computing resources.
